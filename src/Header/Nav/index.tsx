            ...
            <div className="absolute left-0 top-full pt-2 hidden group-hover:block w-64">
                <div className="bg-white dark:bg-zinc-900 shadow-lg rounded-md p-4 border border-zinc-200 dark:border-zinc-800 flex flex-col gap-2">
                     {item.items?.map((subItem: any, k: number) => {
                         if (subItem.blockType === 'singleLink') {
                             return <CMSLink key={k} {...subItem.link} appearance="link" className="block w-full text-left" />
                         }
                         if (subItem.blockType === 'collectionMenu') {
                             return <div key={k} className="mt-2 first:mt-0">
                                <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2 px-2">{subItem.linkLabel}</div>
                                <CollectionSubMenu item={subItem} />
                             </div>
                         }
                         return null
                     })}
...
const CollectionSubMenu = ({ item }: { item: any }) => {
    const getHref = (slug: string) => {
        if (item.collectionSlug === 'industry-solutions') return `/solutions/${slug}`
