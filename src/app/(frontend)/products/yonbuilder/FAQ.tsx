...
const faqs = [
  {
    ...
    answer: '完全适合。YonBuilder 支持“零代码、低代码、全代码”的一体化开发模式。对于轻量级应用，业务人员可以通过零代码快速搭建；对于复杂的核心业务系统，专业开发者可以利用低代码+全代码的方式进行深度定制，同时平台提供微服务架构、容器化部署等企业级能力支撑。'
  },
  {
    question: '如果平台功能不满足需求，支持自定义代码扩展吗？',
    answer: '支持。YonBuilder 提供了强大的脚本引擎和插件扩展机制。前端支持编写 JS 逻辑，后端支持 Java/Python/NodeJS 等语言编写业务逻辑，同时可以通过 Open API 对接外部系统能力，确保能够满足 100% 的个性化需求。'
  },
  {
    question: '平台的数据安全性如何保障？',
    answer: '平台遵循企业级安全标准构建。在数据层面，提供字段级加密、脱敏存储；在传输层面，全链路 HTTPS 加密；在访问层面，支持细粒度的 RBAC 权限控制和多因素认证。此外，YonBuilder 已通过等保三级、ISO27001 等多项权威安全认证。'
  },
  {
    question: '是否支持私有化部署？',
