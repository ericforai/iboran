...
const results = [
  {
    ...
    value: '3天',
    label: '结账周期',
    description: '实现结账透明化，月度财务关账时间由原来的 10 天以上缩短至 3 天内。'
  },
  {
    icon: Award,
    value: '40%',
...
export default function Results() {
  return (
    <section className="py-24 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white relative overflow-hidden">
      ...
      {/* Decorative elements */}
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-green-500/20 rounded-full blur-[150px]" />
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {results.map((result, index) => {
            ...
            return (
              <div
                key={index}
                className="group text-center p-10 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 hover:border-green-500/50 hover:bg-white/10 transition-all duration-300"
              >
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-green-500/20 to-emerald-500/20 flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform duration-300">
                  <Icon className="w-8 h-8 text-green-400" />
                </div>
                <div className="text-5xl font-bold text-white mb-3 group-hover:text-green-400 transition-colors duration-300">
                  {result.value}
                </div>
