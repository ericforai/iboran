...
const results = [
  {
    ...
    value: '数据打通',
    label: '消除信息孤岛',
    description: '打通ERP、SRM、集团财务、合并报表等系统壁垒，实现项目管理、合同管理、费用管理流程一体化，提升信息传递时效与准确性。'
  },
  {
    icon: Target,
    value: '全周期管控',
    label: '资金服务闭环',
    description: '实现企业支付与售后服务系统有效联动，建立资金流与服务流闭环管理，支撑复杂项目全生命周期精细化管控。'
  },
  {
    icon: ShieldCheck,
    value: '业财融合',
    label: '集团协同升级',
    description: '构建统一数字化底座，实现高实时性、高集成性数据流转，推动业财深度融合与集团化资源协同，全面支撑战略决策。'
  }
]
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {results.map((result, index) => {
            ...
            return (
              <div
                key={index}
                className="group text-center p-10 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 hover:border-blue-500/50 hover:bg-white/10 transition-all duration-300"
              >
