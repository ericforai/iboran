...
const values = [
  { 
    icon: Target, 
    title: '使命 Mission', 
    desc: '懂变，才安稳。泊冉，13年未雨绸缪。助力企业在变局中实现稳健增长与数智化跨越。' 
  },
  { 
    icon: ShieldCheck, 
    title: '核心实力 Strength', 
      ...
      <section className="relative bg-slate-900 min-h-[70vh] flex items-center py-20 overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            ...
            >
              <div className="inline-block px-4 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-sm font-bold mb-8 backdrop-blur-md">
                懂变，才安稳。泊冉，13年未雨绸缪。
              </div>
      ...
      <section className="py-16 bg-white border-b border-slate-100">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { label: '服务经验', value: '13+', unit: '年', sub: '始于 2012' },
              ...
              <div key={stat.label} className="text-center group">
                <div className="text-3xl md:text-4xl font-black text-slate-900 mb-1 group-hover:text-blue-600 transition-colors">
                  {stat.value}<span className="text-lg font-bold ml-1">{stat.unit}</span>
                </div>
      ...
      <section className="py-24 bg-white overflow-hidden">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-9 gap-8 items-center opacity-60">
             {clients.map((client) => (
               <div key={client} className="flex flex-col items-center gap-3 group grayscale hover:grayscale-0 transition-all duration-500">
                  <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center text-[10px] font-bold text-slate-400 border border-slate-100 group-hover:border-blue-200 group-hover:bg-blue-50 group-hover:text-blue-600 transition-all">
                    LOGO
                  </div>
                  <span className="text-xs font-medium text-slate-500 group-hover:text-slate-900">{client}</span>
               </div>
             ))}
          </div>
          ...
          <div className="mt-20 pt-16 border-t border-slate-100 flex flex-wrap justify-center gap-x-10 gap-y-6">
            ...
            <Link href="/solution/business/accounting" className="text-sm text-slate-400 hover:text-blue-600 transition-colors"># 业财一体化</Link>
