      ...
      <div className="w-16 md:w-48 bg-[#001529] text-slate-400 flex flex-col border-r border-slate-800">
        ...
        <div className="p-2 space-y-1">
          <div 
            ...
            className={`px-3 py-2 rounded flex items-center gap-2 cursor-pointer transition-colors ${activeTab === 'overview' ? 'bg-[#E60012] text-white' : 'hover:bg-slate-800'}`}
          >
            <Globe size={14} /> <span className="hidden md:inline">全球司库</span>
          </div>
          <div 
            onClick={() => setActiveTab('accounting')}
            className={`px-3 py-2 rounded flex items-center gap-2 cursor-pointer transition-colors ${activeTab === 'accounting' ? 'bg-[#E60012] text-white' : 'hover:bg-slate-800'}`}
          >
            <Activity size={14} /> <span className="hidden md:inline">智能核算</span>
          </div>
          <div 
            onClick={() => setActiveTab('invoice')}
            className={`px-3 py-2 rounded flex items-center gap-2 cursor-pointer transition-colors ${activeTab === 'invoice' ? 'bg-[#E60012] text-white' : 'hover:bg-slate-800'}`}
          >
            <ShieldCheck size={14} /> <span className="hidden md:inline">票据档案</span>
          </div>
      ...
      <div className="flex-1 bg-slate-50 flex flex-col">
        ...
        <div className="h-12 bg-white border-b border-slate-200 flex items-center justify-between px-4 shadow-sm">
          <div className="font-bold text-slate-700 flex items-center gap-2">
            财务云驾驶舱 / {activeTab === 'overview' ? '全球资金监控' : '实时智能核算'}
          </div>
          <div className="flex items-center gap-3">
            <span className="text-slate-400 text-[10px]">最后更新: 刚刚</span>
