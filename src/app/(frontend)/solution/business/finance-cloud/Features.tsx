    ...
    groups: [
      ...
      },
      {
        ...
        features: [
          ...
          { title: "票税表一体化", desc: "开票数据自动关联税务申报，确保账实一致。" }
        ]
      }
    ]
  },
    ...
    groups: [
      {
        ...
        features: [
          ...
          { title: "电子化整理", desc: "自动勾选、装册、编号，完全符合无纸化要求。" },
    ...
    <section className="py-24 bg-white relative overflow-hidden">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-3xl font-bold text-[#1F2329] mb-4">YonBIP 财务云核心能力</h2>
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-3 px-8 py-4 rounded-xl font-bold transition-all duration-300 ${
                activeTab === tab.id
                  ? 'bg-[#E60012] text-white shadow-xl shadow-red-100 scale-105'
        ...
        {/* Tab Content */}
        <div className="bg-[#F7F8FA] rounded-3xl p-8 lg:p-12 min-h-[500px]">
          <AnimatePresence mode="wait">
            {tabs.map((tab) => tab.id === activeTab && (
              ...
              >
                <div className="mb-12">
                  <h3 className="text-2xl font-bold text-[#1F2329] mb-2">{tab.label}</h3>
                  <p className="text-[#0052D9] font-medium">{tab.description}</p>
                </div>

                <div className="grid lg:grid-cols-2 gap-12">
                  {tab.groups.map((group, groupIdx) => (
                    <div key={groupIdx}>
                      <h4 className="flex items-center gap-2 text-lg font-bold text-[#1F2329] mb-6 border-b border-slate-200 pb-2">
                        <Layers size={20} className="text-[#E60012]" />
                        {group.title}
                      </h4>
                      <div className="space-y-6">
                        {group.features.map((feature, featureIdx) => (
                          <div 
                            key={featureIdx}
                            className="bg-white p-6 rounded-2xl hover:shadow-md transition-shadow group flex items-start gap-4"
                          >
                            <div className="w-10 h-10 rounded-lg bg-blue-50 flex items-center justify-center text-[#0052D9] shrink-0 group-hover:bg-[#E60012] group-hover:text-white transition-colors">
                              <Calculator size={20} />
                            </div>
                            <div>
                              <h5 className="font-bold text-[#1F2329] mb-1">{feature.title}</h5>
