    ...
    <section className="py-24 bg-[#001529] text-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-4 gap-8">
          {values.map((item, idx) => (
            <div key={idx} className="bg-white/5 backdrop-blur-sm border border-white/10 p-8 rounded-2xl hover:bg-white/10 transition-all text-center group">
              <div className="w-12 h-12 bg-blue-500/20 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:bg-blue-500/40 transition-colors">
                <item.icon size={24} className="text-blue-400" />
              </div>
              <div className="text-4xl font-extrabold text-white mb-2">{item.value}</div>
              <div className="text-blue-400 font-bold mb-3">{item.label}</div>
