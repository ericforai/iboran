...
export default function MESDashboardMockup() {
  return (
      ...
      <div className="flex-1 flex flex-col bg-[#0f172a] relative z-10">
        ...
        <div className="h-10 border-b border-slate-800 flex items-center justify-between px-4 bg-[#020617]/50">
          <div className="flex items-center gap-2">
             ...
             <span className="text-white">MES 生产指挥中心</span>
          </div>
        ...
        <div className="p-4 grid grid-cols-12 grid-rows-6 gap-3 flex-1 overflow-hidden">
           ...
           <div className="col-span-3 row-span-2 bg-[#1e293b] rounded-lg border border-slate-700 p-3 flex flex-col justify-between relative overflow-hidden">
             ...
             <div className="text-2xl font-bold text-white z-10">89.2<span className="text-sm text-slate-500 font-normal">%</span></div>
             <div className="flex items-center gap-1 text-[10px] text-green-400 z-10">
               <span className="rotate-180">▼</span> +3.1% 较目标
             </div>
             {/* BG Chart */}
           ...
           <div className="col-span-3 row-span-2 bg-[#1e293b] rounded-lg border border-slate-700 p-3 flex flex-col justify-between">
             <div className="flex justify-between items-start">
               <span className="text-slate-400 text-[10px] uppercase tracking-wider">今日产量</span>
               <Truck size={14} className="text-purple-500" />
             </div>
             <div className="text-2xl font-bold text-white">2,850<span className="text-sm text-slate-500 font-normal"> 件</span></div>
             <div className="w-full bg-slate-700 h-1 mt-2 rounded-full overflow-hidden">
               <div className="w-[82%] bg-purple-500 h-full rounded-full"></div>
             </div>
           </div>

           <div className="col-span-3 row-span-2 bg-[#1e293b] rounded-lg border border-slate-700 p-3 flex flex-col justify-between">
             <div className="flex justify-between items-start">
               <span className="text-slate-400 text-[10px] uppercase tracking-wider">直通率 (FPY)</span>
               <ShieldCheck size={14} className="text-green-500" />
             </div>
             <div className="text-2xl font-bold text-white">99.5<span className="text-sm text-slate-500 font-normal">%</span></div>
                  ...
                  >
                    ...
                    <div>
                       <div className={`text-[10px] font-bold ${i === 1 ? 'text-yellow-100' : 'text-slate-300'}`}>
                         {i === 1 ? 'CNC-04 刀具寿命预警' : `工单 WO-2024-${800 + i} 完工`}
                       </div>
                       <div className="text-[9px] text-slate-500">{i * 5 + 2} 分钟前</div>
                    </div>
                  </motion.div>
                ))}
           ...
           <div className="col-span-9 row-span-4 bg-[#1e293b] rounded-lg border border-slate-700 p-4 relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-4 relative z-10">
                 <div className="text-white font-bold">车间 1 - 精加工单元</div>
                 <div className="flex gap-2 text-[10px]">
                    <span className="px-2 py-0.5 bg-blue-900/30 text-blue-300 border border-blue-800 rounded">自动调度中</span>
                    <span className="px-2 py-0.5 bg-slate-700 text-slate-300 rounded">白班</span>
                 </div>
              </div>

              {/* Simplified Gantt / Line Visualization */}
              <div className="relative z-10 mt-6 space-y-6">
                 {['工序 10: 粗铣', '工序 20: 精车', '工序 30: 磨削'].map((stage: any, idx: number) => (
                    <div key={idx} className="flex items-center gap-4">
                       ...
                       <div className="w-8 text-[10px] text-white">{90 - idx * 15}%</div>
                    </div>
                 ))}
              </div>
           </div>
