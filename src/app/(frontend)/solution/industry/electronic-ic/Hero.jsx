...
var DemoRequestModal_1 = require("@/components/DemoRequestModal");
function Hero() {
    var _a = (0, react_1.useState)(false), isDemoOpen = _a[0], setIsDemoOpen = _a[1];
    return (<>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            <div className="flex-1 text-center lg:text-left">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-white text-sm font-medium mb-6">
                <lucide_react_1.Cpu size={16} className="text-cyan-400"/>
                电子IC行业解决方案
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 justify-center lg:justify-start">
                <button onClick={function () { return setIsDemoOpen(true); }} className="px-8 py-3.5 bg-[#E60012] text-white rounded-md font-bold hover:bg-red-700 transition shadow-lg shadow-red-900/30 flex items-center gap-2 group">
                  预约行业专家咨询
                  <lucide_react_1.ArrowRight size={18} className="group-hover:translate-x-1 transition-transform"/>
                </button>
              ...
              <div className="mt-8 flex items-center justify-center lg:justify-start gap-6 text-sm text-slate-400">
                <span className="flex items-center gap-1.5"><lucide_react_1.CheckCircle size={16} className="text-green-400"/> 委外协同</span>
                <span className="flex items-center gap-1.5"><lucide_react_1.CheckCircle size={16} className="text-green-400"/> 批号追溯</span>
                <span className="flex items-center gap-1.5"><lucide_react_1.CheckCircle size={16} className="text-green-400"/> 成本模拟</span>
              </div>
            </div>
                ...
                <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-8 border border-white/10">
                  <div className="text-center mb-6">
                    <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-xl shadow-lg shadow-cyan-500/30 mb-4">
                      <lucide_react_1.Cpu size={40} className="text-white"/>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
            { icon: lucide_react_1.Factory, name: 'MES', desc: '生产执行' },
            { icon: lucide_react_1.Layers, name: '委外', desc: '协同管理' },
            { icon: lucide_react_1.Package, name: '追溯', desc: '批号/刻号' },
            { icon: lucide_react_1.Calculator, name: '成本', desc: '效益模拟' },
            { icon: lucide_react_1.TrendingUp, name: 'YMS', desc: '良率管理' },
            { icon: lucide_react_1.Cpu, name: 'WIP', desc: '在制管理' },
        ].map(function (module, idx) { return (<div key={idx} className="bg-white/5 rounded-lg p-3 text-center border border-white/10 hover:bg-white/10 transition">
      ...
      <DemoRequestModal_1.DemoRequestModal isOpen={isDemoOpen} onClose={function () { return setIsDemoOpen(false); }} source="industry-electronic-ic"/>
    </>);
}
