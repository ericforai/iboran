...
function IndustryCases() {
    return (<section className="py-24 bg-[#F7F8FA]">
      <div className="container mx-auto px-4">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {cases.map(function (caseItem, idx) { return (<div key={idx} className="bg-white p-10 rounded-3xl shadow-sm border border-slate-100 hover:shadow-xl transition-all group">
              {/* 企业信息头 */}
              <div className="flex items-start justify-between mb-8 border-b border-slate-50 pb-6">
                <div className="flex gap-4 items-center">
                  <div className="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center">
                    <lucide_react_1.Cpu className="w-6 h-6 text-[#0052D9]"/>
                  </div>
                  <div>
              ...
              {/* 客户证言 */}
              {caseItem.quote && (<div className="mt-8 pt-8 border-t border-slate-50 relative">
                  <lucide_react_1.Quote className="absolute top-6 left-0 w-4 h-4 text-slate-200"/>
