...
为避免统计数据失真，百度统计在本项目中遵循以下规则：

1. 百度统计脚本只允许在正式域名 `iboran.com` 和 `www.iboran.com` 下加载，本地开发、测试环境、预发环境均不得注入。
2. 百度统计代码统一放在前台根布局的 `<head>` 中，确保全站覆盖，当前实现位置为 `src/app/(frontend)/layout.tsx`。
3. `hm.js?` 后面的参数必须是百度统计后台提供的 32 位 Site ID，不得复用爱番番的 `siteToken`、聊天链接参数或其他百度系产品标识。
4. 爱番番咨询能力与百度统计分开维护。统计脚本只负责页面与事件统计，咨询入口继续使用独立的爱番番聊天链接，不在统计脚本中混接商桥逻辑。
...
3. 确认网络请求未被拦截
4. 百度统计有 5-10 分钟延迟
5. 确认当前访问域名是否为 `iboran.com` 或 `www.iboran.com`；本地和非正式域名不会加载百度统计
6. 确认 `hm.js?` 后面的值来自百度统计后台，而不是爱番番 `siteToken` 或聊天链接参数
